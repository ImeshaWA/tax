...
class TaxCalculator { 
  ...
  double calculateTotalEmploymentIncome() { 
    return _service.employmentCategories.values.fold(0.0, (sum, val) => sum + 
val); 
  } 
 
  // Calculate Total Annual Business Income 
  double calculateTotalBusinessIncome() { 
    return _service.businessCategories.values.fold(0.0, (sum, val) => sum + 
val); 
  } 
 
  // Calculate Total Annual Investment Income 
  double calculateTotalInvestmentIncome() { 
    double total = _service.investmentCategories.values.fold(0.0, (sum, val) 
=> sum + val); 
  ...
  double calculateTotalForeignIncome() { 
    return _service.foreignIncomeCategories.values.fold(0.0, (sum, val) => 
sum + val); 
  } 
  ...
  double calculateRentRelief() { 
    if (_service.isMaintainedByUser == true || _service.rentMaintainedByUser 
== true) { 
      return (_service.totalRentIncome * 0.25) + (_service.rentBusinessIncome 
* 0.25); 
    } 
    return 0.0; 
  } 
  ...
  double calculateSolarRelief() { 
    double solarBalance = _service.solarInstallCost - (600000 * 
_service.solarReliefCount); 
  ...
  double calculateTaxableIncome() { 
    ...
    return assessableIncome - calculateTotalQualifyingPayments() - 
calculateTotalRelief(); 
  } 
  ...
  double calculateTaxableIncomeWithoutForeign() { 
    ...
    return taxableIncome - foreignIncome < 0 ? 0 : taxableIncome - 
foreignIncome; 
  } 
  ...
  double calculateFinalTaxLiability() { 
    return calculateTaxLiabilityWithoutForeign() + 
calculateTaxLiabilityForeign(); 
  } 
  ...
  double calculateTaxPayable() { 
    double payable = calculateFinalTaxLiability() - calculateAnnualAPIT() - 
_service.foreignTaxCredits; 
