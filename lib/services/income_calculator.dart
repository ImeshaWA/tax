//services/income_calculator.dart 
 
class IncomeCalculator { 
  // Employment Income 
  static double calculateUpToNowEmployment(List<double?> 
savedEmploymentIncome, int monthIndex) { 
    double sum = 0.0; 
    for (int i = 0; i <= monthIndex; i++) { 
      sum += savedEmploymentIncome[i] ?? 0.0; 
    } 
    return sum; 
  } 
 
  static double calculateAnnualEmployment(List<double?> 
savedEmploymentIncome) { 
    return savedEmploymentIncome.fold(0.0, (sum, val) => sum + (val ?? 0.0)); 
  } 
 
  // Dynamic Income (Business, Investment, Other) 
  static double calculateUpToNowDynamic(List<List<double>> 
savedDynamicIncome, int monthIndex) { 
    double sum = 0.0; 
    for (int i = 0; i <= monthIndex; i++) { 
