...
class TaxComputationService {
  ...
  // 11. Calculate Total Withholding Tax
  double totalWithholdingTax() {
    return calculateTotalInterestWHT() + calculateTotalBusinessRentWHT();
  }

  // 12. Calculate Total Interest Withholding Tax
  double calculateTotalInterestWHT() {
    double interestIncome =
        service.investmentCategories["Interest Income"] ?? 0.0;
    return interestIncome * 0.1;
  }

  // 13. Calculate Total Business Rent Withholding Tax
  double calculateTotalBusinessRentWHT() {
    return service.rentBusinessIncome * 0.1;
  }

  // 14. Calculate Quarterly Installment Payments
  double firstQuarterlyInstallment() {
    double netAmount = taxPayable() - totalWithholdingTax();
    return netAmount / 4;
  }

  double secondQuarterlyInstallment() {
    double netAmount = taxPayable() - totalWithholdingTax();
    return netAmount / 3;
  }

  double thirdQuarterlyInstallment() {
    double netAmount = taxPayable() - totalWithholdingTax();
    return netAmount / 2;
  }

  double fourthQuarterlyInstallment() {
    double netAmount = taxPayable() - totalWithholdingTax();
    return netAmount;
  }
}
